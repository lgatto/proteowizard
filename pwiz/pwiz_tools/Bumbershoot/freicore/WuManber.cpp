...
// Modified by Surendra Dasari (Vanderbilt University)
...
unsigned char WuManber::rchExtendedAscii[] = { 
    ...
    map<string,size_t> WuManber::Search( size_t TextLength, const char *Text, const vector<const char *> &patterns ) {

        map<string,size_t> matches;
                ...
                while ( element.end() != iter ) {
                    if ( hash2 == (*iter).PrefixHash ) {  
                        ...
                        if ( 0 == *ixPattern ) {  // we found the end of the pattern, so match found
                            string match(patterns[ (*iter).ix ]);
                            matches[match] = ix-m+1;
