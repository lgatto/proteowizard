...
using namespace Analyst;

AnalystInterface::AnalystInterface(void) :
...
void AnalystInterface::setVerbose(int iVerbose) {
	m_iVerbose = iVerbose;
}

bool AnalystInterface::initInterface(void) {
...
bool AnalystInterface::setInputFile(const std::string& filename, const ProcessingOptions& options) {
	//----------------------------------------
	{
...
void AnalystInterface::setCentroiding(bool centroid, bool tandemCentroid, bool preferVendor) {
	doCentroid_ = centroid;
	doTandemCentroid_ = tandemCentroid;
	preferVendorCentroid_ = preferVendor;
}

void AnalystInterface::setDeisotoping(bool deisotope) {
	doDeisotope_ = deisotope;
}

Scan* AnalystInterface::getScan(void) {
	if (firstTime_) {
		firstTime_ = false;
		return getFirstScan();
	} else {
...
void AnalystInterface::termInterface(void) {
...
int AnalystInterface::getCreator(const std::string &strFilename, std::string &strCreator) {
...
void AnalystInterface::getMSManufacturer(std::string &strManufacturer) {
...
void AnalystInterface::getMSModel(long &lModelId, std::string &strModel) {
	strModel.resize(0);
...
void AnalystInterface::getMSSerialNumber(std::string &strSerialNumber) {
...
void AnalystInterface::getMSHardwareVersion(std::string &strHardwareVersion) {
...
void AnalystInterface::getMSIonisation(std::string &strIonisation) {
	// TODO:
...
void AnalystInterface::getMSMassAnalyzer(std::string &strMassAnalyzer) {
	// TODO:
	// does not seem to find this recorded much
...
void AnalystInterface::getMSDetector(std::string &strDetector) {
	// TODO:
	// does not seem to find this recorded much
	strDetector.resize(0);
}

Scan* AnalystInterface::getFirstScan(void)
{
	initScanIteratorState ();
	if (isValidScan()) {
		// return cache result!!!
...
Scan* AnalystInterface::getNextScan(void)
{
...
bool AnalystInterface::isValidScan(void) {
...
inline void AnalystInterface::determineRetentionTimeType(long lSampleIndex, long lPeriodIndex) {
	m_fUseExperiment0RT = true;
	try {
...
inline void AnalystInterface::processSpectrum(Scan *pScan) {
...
inline void AnalystInterface::processMSSpectrum(Scan *pScan) {
...
inline void AnalystInterface::processMSSpecData(Scan *pScan) {
		...
		if (thresholdIntensity > 0.0) {
			for (long dpi=1; dpi<=lSpecDataPoints; dpi++) {
				...
				if (dpiYVal<thresholdIntensity) {
					...
					pScan->mzArray_[lPeaksCount] = (Scan::MzValueType) dpiXVal;
					pScan->totalIonCurrent_ += (pScan->intensityArray_[lPeaksCount] = (Scan::IntenValueType) dpiYVal);
					lPeaksCount++;

					if (pScan->basePeakIntensity_ < dpiYVal) {
						pScan->basePeakMZ_ = (Scan::MzValueType) dpiXVal;
						pScan->basePeakIntensity_ = (Scan::IntenValueType) dpiYVal;
					}
				}
			}
		} else {
...
inline void AnalystInterface::centroidSpecData(bool doDeisotope, double &maxIntensity) {
...
inline void AnalystInterface::processMRMSpectrum(Scan *pScan) {
		...
		for (long dpi=1; dpi<=lSpecDataPoints; dpi++) {
			...
			if (dpiYVal<thresholdIntensity) {
				...
				pMRMScan->mzArray_[0] 
					...
					= (Scan::MzValueType) ipMassRange->GetQstepMass();
				pMRMScan->intensityArray_[0] = (Scan::IntenValueType) dpiYVal;
				pMRMScan->totalIonCurrent_ += (Scan::IntenValueType) dpiYVal;

				pMRMScan->basePeakIntensity_ = (Scan::IntenValueType) dpiYVal;
				pMRMScan->basePeakMZ_ 
					= pMRMScan->startMZ_ 
					= pMRMScan->endMZ_ 
					= (Scan::MzValueType) ipMassRange->GetQstepMass();
				pMRMScan->precursorList_.back().intensity_ = 0;
				pMRMScan->precursorList_.back().MZ_ = (Scan::MzValueType) ipMassRange->GetQstartMass();
...
inline void AnalystInterface::getMSRunTime () {
...
void AnalystInterface::computeScansCount () {
	if (0>=totalNumScans_) {
...
bool AnalystInterface::getExperimentsInfo(long lSampleIndex, long lPeriodIndex, std::vector<MSScanType> &experimentsScanType, std::vector<long> &experimentsPolarity, std::vector<long> &experimentsUseMCAScans) {
	bool fToWarn = false;
	long lActualNumberOfExperiments = m_ipFMANWiffFile->GetNumberOfExperiments(lSampleIndex,lPeriodIndex);
...
void AnalystInterface::mergeScans (
...
ScanPtr AnalystInterface::instantiateMergedScans (long lSampleIndex, long lPeriodIndex, const MergedScan &mergedScan) {
...
void AnalystInterface::getSamplePrecursorInfo (long lSampleIndex, long lPeriodIndex) {
...
void AnalystInterface::getSamplePrecursorInfo (DDEData &ddeData, long lSampleIndex, long lPeriodIndex) {
...
void AnalystInterface::getSampleLog(void) {
...
inline void AnalystInterface::loadMergedScans (long lSampleIndex, long lPeriodIndex) {
...
inline void AnalystInterface::initScanIteratorState () {
...
inline bool AnalystInterface::nextScanIteratorState () {

	ScanIteratorState::IteratorStateType state = m_iteratorState.next ();

	if (m_iVerbose>=VERBOSE_DEBUGDETAIL) {
		std::cout << "DEBUG: nextScanIteratorState("
				<<m_iteratorState.m_lCurrentSampleIndex<<", "
				<<m_iteratorState.m_lCurrentPeriodIndex<<", "
				<<m_iteratorState.m_lCurrentExperimentIndex<<")" 
				<<std::endl;
...
void AnalystInterface::buildInstrumentInfo() {
	// TODO:
...
void AnalystInterface::buildAcquisitionSoftwareInfo() {
...
void AnalystInterface::setVerbose(bool verbose) {
	verbose_ = verbose;
}
