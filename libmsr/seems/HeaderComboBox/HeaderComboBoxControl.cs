...
namespace HeaderComboBox
{
	public partial class HeaderComboBoxControl : UserControl
	{
		...
		private void button1_Click( object sender, EventArgs e )
		{
			...
			Graphics g;
			bool hasScrollBar = (ListBox.Height / (ListBox.Items.Count * ListBox.ItemHeight)) < 1;
			int scrollBarWidth = hasScrollBar ? System.Windows.Forms.SystemInformation.VerticalScrollBarWidth : 0;
			if( Items.Count > 0 )
			{
				g = ListBox.CreateGraphics();
				string itemText;
				System.Reflection.PropertyInfo displayMember = Items[0].GetType().GetProperty( ListDisplayMember );
				if( displayMember != null )
					itemText = displayMember.GetValue( Items[0], null ).ToString();
				else
					itemText = Items[0].ToString();
				ListBox.Width = g.MeasureString( itemText, ListBox.Font ).ToSize().Width + scrollBarWidth * 2;
			}
			...
			listBoxLocation.X = Math.Max( 0, listBoxLocation.X );
